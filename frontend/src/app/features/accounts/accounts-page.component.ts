...
export class AccountsPageComponent implements OnDestroy {
  ...
  protected submitCreateAccount(payload: {
    name: string;
    type: AccountType;
    ...
    this.accountApi.createAccount(payload).subscribe({
      next: (createdAccount) => {
        const nextAccounts = [...this.accounts(), createdAccount].sort((left, right) =>
      ...
      error: (error: HttpErrorResponse) => {
        ...
        this.submitErrorMessage.set(
          error.error?.message ?? 'Account creation failed. Please review the data and try again.',
        );
      },
    });
  }
  ...
  protected async sharePreparedPaymentDetails(): Promise<void> {
    ...
    try {
      ...
      const shareFile = new File(
        [previewBlob],
        this.buildPaymentDetailsFileName(account.accountNumber),
        {
  ...
  private resolveShareErrorMessage(error: HttpErrorResponse): string {
    ...
    if (error.status === 409) {
      return (
        error.error?.message ?? 'Payment details are not available for this account right now.'
      );
    }
